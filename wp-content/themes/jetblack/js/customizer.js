...
jQuery( document ).ready(function($) {
	...
	// Make Main Content section in Sortable Section unclickable.
	$('#accordion-section-jetblack_ss_main_content h3').unbind("click");
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					jetblackAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			jetblackGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				jetblackGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			jetblackGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	function jetblackAppendRow($element, defaultValue) {
	...
	function jetblackGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  jetblackGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function jetblackGetAllImageCheckboxes($element) {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].jetblacktinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].jetblacktinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].jetblackmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['jetblack-upsell'] = api.Section.extend( {
