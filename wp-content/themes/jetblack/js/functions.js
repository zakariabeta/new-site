/* global jetblackScreenReaderText */
/**
...
(function($) {
	...
	function initMainNavigation(container) {
		...
		var dropdownToggle = $('<button />', {
			...
			.append(jetblackScreenReaderText.icon)
			.append($('<span />', {
				'class': 'screen-reader-text',
				text: jetblackScreenReaderText.expand
			}));
		...
		container.find('.current-menu-ancestor > button')
			...
			.text(jetblackScreenReaderText.collapse);
		...
		container.find('.dropdown-toggle').on( 'click', function(e) {
			...
			// jscs:enable
			screenReaderSpan.text(screenReaderSpan.text() === jetblackScreenReaderText.expand ? jetblackScreenReaderText.collapse : jetblackScreenReaderText.expand);
		});
	}
	...
	(function() {
		...
		menuToggle.on('click.jetblack', function() {
	...
	(function() {
		...
		function toggleFocusClassTouchScreen() {
			if (window.innerWidth >= 910) {
				$(document.body).on('touchstart.jetblack', function(e) {
					if (!$(e.target).closest('.main-navigation li').length) {
						$('.main-navigation li').removeClass('focus');
					}
				});
				siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.jetblack', function( e ) {
				...
				siteNavigation.find('.menu-item-has-children > a').unbind('touchstart.jetblack');
			}
		}

		if ('ontouchstart' in window) {
			$(window).on('resize.jetblack', toggleFocusClassTouchScreen);
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find('a').on('focus.jetblack blur.jetblack', function() {
	...
	$(document).ready(function() {
		$(window).on('load.jetblack resize.jetblack', function() {
			if (window.innerWidth < 910) {
